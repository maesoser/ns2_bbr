...
QuicTime::Delta BbrSender::TimeUntilSend(QuicTime now, QuicByteCount bytes_in_flight) {
...
QuicBandwidth BbrSender::PacingRate(QuicByteCount bytes_in_flight) const {
  if (pacing_rate_.IsZero()) {
    return kHighGain * QuicBandwidth::FromBytesAndTimeDelta(initial_congestion_window_, GetMinRtt());
  }
  return pacing_rate_;
}
...
void BbrSender::CalculateCongestionWindow(QuicByteCount bytes_acked) {
  ...
  if (rtt_variance_weight_ > 0.f && !BandwidthEstimate().IsZero()) {
    target_window += rtt_variance_weight_ * rtt_stats_->mean_deviation() * BandwidthEstimate();
  } else if (FLAGS_quic_reloadable_flag_quic_bbr_ack_aggregation_bytes && is_at_full_bandwidth_) {
    QUIC_FLAG_COUNT_N(quic_reloadable_flag_quic_bbr_ack_aggregation_bytes, 2, 2);
    target_window += max_ack_height_.GetBest();
  } else if (FLAGS_quic_reloadable_flag_quic_bbr_ack_aggregation_bytes2 && is_at_full_bandwidth_) {
    QUIC_FLAG_COUNT_N(quic_reloadable_flag_quic_bbr_ack_aggregation_bytes2, 2, 2);
